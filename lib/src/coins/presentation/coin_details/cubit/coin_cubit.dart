...
class CoinCubit extends Cubit<CoinState> {
  ...
  Future<void> getCoinById(String id, double convertedRate) async {
    emit(GettingCoin());

    final result = await _getCoinById(GetCoinByIdParams(id: id));

    result.fold((failure) => emit(CoinError(failure.toString())), (coin) {
      emit(CoinLoaded(coin.copyWith(
          convertedPrice: convertNumberString(
              (coin.usdPrice * convertedRate).toString()))));
    });
  }
}
